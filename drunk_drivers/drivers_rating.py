from sqlite3_helper import sqlite3_create, sqlite3_run #using library sqlite3_helper.py to communicate with mysqlite3 database
import os
import re
...
def colored(r, g, b, text): #text color changing func
    return "\033[38;2;{};{};{}m{} \033[38;2;255;255;255m".format(r, g, b, text)

def negative_review(driver_id): 
    do_you_like = 5 #if user had negaitve mind about trip, the driver get automatically 5 points. 
    print("Ouch, this is bad... So on a 1-10 scale, how would you rate the driving style of the driver?")
    while True:
        try:
            style_score	 = int(input())
        except ValueError: #if user insert a space or nothing, it prints message on next line 
...
def dev_mode_new_driver(): #func to add new driver. Available only in developer's mode
...
def dev_exit(): #Message that prints out after leaving from developer's mode
...
def positive_review(driver_id):
    do_you_like = 10 #if user had positive mind about trip, the driver get automatically 5 points. 
            ...
            while True:
                ...
                if style_score > 0 and style_score < 11:
                    ...
                    while True:
                        ...
                        if behaviour_score > 0 and behaviour_score < 11:
                            rate_list = [do_you_like, style_score, condition_score, behaviour_score]
                            avg_trip_score = round(mean(rate_list)) #Take average from data input and round it
                            trips(do_you_like, condition_score, style_score, behaviour_score, driver_id, avg_trip_score)
                            print("Thanks for you review! Closing program...")
                            exit()
                        else:
...
def remove_sym(name): #removing odd symbols from extracted data
    name = re.sub(r"[\[\]]",'',name)
    name = name.strip("()")
    name = name.replace("'", "")
    name = name.replace(",", "")
...
def program_exit(): #message that prints before leaving the programm
    print("Closing program...")

def dev_change_id(driver_id): #really important func. By code creator's idea, the customer app or website change this value, to choose correct driver.
                              #Each driver have own driver_id in database, so if you want change driver, set this driver_id by this func!
...
def driver_info(driver_id): #printing data about driver by driver_id
    ...
    if avg_score <= 5.9: #driver have bad rating
        avg_score_colored = colored(225, 0, 0, str(avg_score)) #printing colored text (in this line, it prints red text)
        avg_score_colored = avg_score_colored.replace(" ", "")
    elif avg_score >= 6 and avg_score <= 7.9: #driver have good rating
        avg_score_colored = colored(255, 191, 0, str(avg_score)) #printing yellow text
        avg_score_colored = avg_score_colored.replace(" ", "")
    elif avg_score >= 8:    #driver have awesome rating
        avg_score_colored = colored(0, 132, 80, str(avg_score)) #printing green text
        avg_score_colored = avg_score_colored.replace(" ", "")
...
def run_query(query): #execute mysql command, to communicate with db
        sqlite3_output = sqlite3_run(sqlite3_file, query)

        return sqlite3_output

def sqlite3_check(): #checking, if db exist. If not, func create it by schema
    ...
    for schema in schema_list:
        ...
        else: #if schema not found, it prints error
            output = f'Error: Required file "{schema}" is not exist'

            return output


def driver(driver_name, driver_family_name, driver_plate_number):  #func to communicate with db about adding a driver
...
def trips(do_you_like, condition_score, style_score, behaviour_score, driver_id, avg_trip_score): #func to communicate with db about adding a review about trip
    if sqlite3_check() == None:
...
def main():
    driver_id = 1 # Each driver have own id on database, so if we need change a current driver, we need change this id by typing into console "developer_mode" and "change_id"
                  # by creator's idea, this value change the customer's app.
    print("Did you enjoy the ride? (yes/no)\nIf you need more info about driver, type \"info\"\nIf you need close the program, type \"exit\"")
    ...
    while True:
        ...
        elif user_input == "developer_mode": #secret mode for technical solutions
            return_developer_mode()
            while True:
                user_input = input()
                if user_input == "add":
